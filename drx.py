...
app = Flask(__name__)
API_SECRET = "NEEL_PRIVATE_API" #
TELEGRAM_CHANNEL = "https://t.me/+kmns0mkOVgo0Mjk1"
...
def handle_expired_script(message):
    bot.reply_to(
        message, 
        "⚠️ <b>SCRIPT EXPIRE OWNER TO BUY PREMIUM SCRIPT @co_neel</b>", 
        parse_mode="HTML")
        
def is_script_expired():
    return datetime.now() > SCRIPT_EXPIRY
# --- [ ADD TO CONFIGURATION SECTION ] ---
DOWNLOAD_PASSWORD = "co-neel" # Change this to your desired password
# --- [ DATABASE LOGIC ] ---
def load_db():
...
def handle_genkey_advance(message):
    ...
    for _ in range(amount):
        ...
        key = "CO-NEEL-" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=10))
...
def handle_redeem(message):
    ...
    if len(cmd) != 2:
        usage_msg = (
            ...
            f"📝 <b>𝐄𝐱𝐚𝐦𝐩𝐥𝐞:</b>\n"
            f"<code>/redeem CO-NEEL-ABC123XYZ</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
...
def view_plans_pro(message):
    ...
    plan_msg = (
        ...
        f"📅 7 Days (Week): ₹210 (Free 2Hour)\n"
        f"📅 15 Days (2 Week): ₹310 (Free 12Hour)\n"
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            bot_info = bot.get_me()
            print(f"\033[1;32m[+] CO-NEEL BOT SUCCESSFUL ✅ STATED @{bot_info.username}\033[0m")
